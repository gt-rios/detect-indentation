...
export default {
  ...
  setIndentation(editor, { softTabs, tabLength }) {
    ...
    if (tabLength === null) tabLength = atom.config.get('editor.tabLength', { scope });
    editor.setTabLength(tabLength);
    this.observedEditors.add(editor);
    this.indentationStatusView.update(editor);
  },

  fixIndentation(editor) {
    let frames = 0;
    const softTabs = editor.getSoftTabs();
    const tabLength = editor.getTabLength();
    const checkIndentation = () => {
      frames++;
      if (frames < 30 && editor.getSoftTabs() === softTabs && editor.getTabLength() === tabLength) {
        window.requestAnimationFrame(checkIndentation);
      } else {
        this.setIndentation(editor, { softTabs, tabLength });
      }
    };
